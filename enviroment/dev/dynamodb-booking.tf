module "dynamodb_table_booking" {
  ...
  name           = "${var.enviroment}-${var.api_core_booking}-${var.api_endpoints_booking}-table"
  hash_key       =  "id"
  write_capacity = 1
  read_capacity  = 1
  pk_name        = "id"
...
module "parameter-ssm-table_name_booking" {
  ...
  name           = "${var.enviroment}-parameter-${var.api_core_booking}-${var.api_endpoints_booking}-table_name"
  type           = "String"     
